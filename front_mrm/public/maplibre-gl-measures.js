...
export default class MeasuresControl {
      ...
      styles: [
        ...
        // line stroke
        {
          ...
          paint: {
            ...
            'line-width':
              this.options?.style?.lengthMeasurement?.lineWidth ?? 4,
          },
        },
        // polygon fill
        {
        ...
        // Intersection points marker
        {
          id: 'gl-draw-polygon-and-line-intersection-marker',
          type: 'symbol',
          filter: [
            'all',
            ['==', 'meta', 'intersection'],
        ...
        // polygon mid points
        {
          ...
          paint: {
            ...
            'circle-color':
              this.options?.style?.common?.midPointColor ?? '#232253',
          },
        },
        ...
        // vertex points
        {
          ...
          paint: {
            'circle-radius': 6,
            'circle-color': '#232253', //ato
            'circle-stroke-color': '#ffffff', // Couleur du contour
            'circle-stroke-width': 2, // Épaisseur du contour
          },
        },
  ...
  onAdd(map) {
    this._map = map;
    if (this._map) {
      this._map.addControl(this._drawCtrl, 'top-right');
      this._initControl();
  ...
  _initControl() {
    ...
    // this.initClearBtn()
    this._drawCtrl.changeMode(this._drawCtrl.modes.DRAW_LINE_STRING);
  }
  ...
  _recreateSourceAndLayers() {
    ...
    if (!this._map.getLayer(DRAW_LABELS_LAYER_ID))
      console.log('DRAW_LABELS_LAYER_ID', DRAW_LABELS_LAYER_ID);
    this._map.addLayer({
      ...
      layout: {
        ...
        'text-size': [
          ...
          18.0,
          19,
          20.0, // Change 15.0 to 10.0 or lower
        ],
      },
      paint: {
        'text-color': '#232253',
        ...
        // "text-halo-blur": 2, // Définissez le flou du rayon du fond ici
        // "text-shadow-color": "rgba(0, 0, 0, 0.5)", // Couleur de l'ombre
        // "text-shadow-offset": [1, 1], // Décalage horizontal et vertical de l'ombre
        // "text-shadow-blur": 2,
      },
    });
    if (this._map.getLayer(DRAW_LABELS_LAYER_ID)) {
  ...
  _reorderLayers() {
    if (this._map) {
      ...
      let layerExists = this._map.getLayer(DRAW_LABELS_LAYER_ID);
      if (layerExists) {
        this._map
          .getStyle()
        ...
        // Déplacer la couche uniquement si elle existe dans le style de la carte
        this._map.moveLayer(DRAW_LABELS_LAYER_ID);
      }
    }
  }
  ...
  onRemove() {
    if (this._container) {
      ...
      this._map.removeControl(this._drawCtrl, 'top-left');
