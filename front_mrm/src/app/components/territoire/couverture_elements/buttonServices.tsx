'use client';
import React, { useState } from 'react';

import { twMerge } from 'tailwind-merge';

import { useTranslations } from 'next-intl';

import { useServiceCouvTerritory } from '@/store/serviceCouvTerritory';

interface buttonService {
  className?: string;
  children?: React.ReactNode;
}

export default function ButtonServices({
  className = '',
  children,
}: buttonService) {
  const { selectedServiceCouvTerritory, setselectedServiceCouvTerritory } =
    useServiceCouvTerritory();
  const [isSelected, setIsSelected] = useState(true);
  const couvertureTranslation = useTranslations('couverture');

  const setActiveService = () => {
    setIsSelected(!isSelected);
    selectedServiceCouvTerritory === 'internet'
      ? setselectedServiceCouvTerritory('appel_sms')
      : setselectedServiceCouvTerritory('internet');
  };

  return (
    <div className={twMerge('relative flex w-full', className)}>
      <label className='flex-grow cursor-pointer'>
        <input
          checked={isSelected}
          className='sr-only peer'
          value=''
          type='checkbox'
          onChange={() => setActiveService()}
        />
        <div
          className={`shadow font-paragraphe peer rounded-xl outline-none duration-100 
                    h-12 bg-white py-2 px-3 flex flex-row items-center relative
                    `}
        >
          <div
            className={`absolute w-[calc(50%-12px)] h-9 rounded-lg duration-300 bg-primary z-[5] transition-transform ${
              selectedServiceCouvTerritory === 'internet'
                ? 'transition-transform ease-in-out duration-300 translate-x-[0%]'
                : `transition-transform ease-in-out duration-300 translate-x-[100%]`
            }`}
            data-test='coverage-service-buttons'
          ></div>
          <span
            className={`w-1/2 rounded-md duration-300 text-sm text-center text-gray-400 z-[5] ${
              selectedServiceCouvTerritory === 'internet' ? 'text-white' : ''
            }`}
          >
            {couvertureTranslation('internetServices')}
          </span>
          <span
            className={`w-1/2 rounded-md duration-300 text-sm text-center text-gray-400 z-[5] ${
              selectedServiceCouvTerritory === 'appel_sms' ? 'text-white' : ''
            }`}
          >
            {couvertureTranslation('callSMS')}
          </span>
        </div>
      </label>
      {children}
    </div>
  );
}
