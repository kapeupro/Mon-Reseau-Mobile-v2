...
//                      http://localhost:3801 (matches api/ default port).
//                      In compose it is "/api" (nginx proxies to the api svc).
//
// A dev proxy is provided so `bun run dev` can talk to a local API on :3801
...
export default defineConfig({
  ...
  server: {
    port: 5173,
    host: true,
    // Dev convenience: forward /api to a locally running API on :3801 so the
    // frontend can use a relative base URL during development if desired.
    proxy: {
      "/api": {
        target: process.env.VITE_DEV_API_PROXY ?? "http://localhost:3801",
        changeOrigin: true,
      },
    },
  },
