...
TYPE_IMPORT = "import statistique couverture territoire"
TABLE = "stats_couv_territoires"
TABLE_FIELDS = [
    "techno",
    "operateur",
    "territoire",
...
class CouvertureStatsTerritoire(InsertCsv):
    ...
    def readSelected(self):
        ...
        self.createtablestatterritoire()
        response, file_copy = self.readcsvfile()

        if file_copy:
            self.copyfile(file_copy)

        self.updatestatcouvterritoire()

        message = response["message"] + " territoire importé avec succès"
    ...
    def updatestatcouvterritoire(self):
    ...
    def updatecodeoperateur(self):
        ...
        query = sql.SQL("""
            ...
            set code_operateur = {schema}.arcep_get_code_operateur(st.operateur, st.territoire)
        """).format(
    ...
    def createtablestatterritoire(self):
        dbcon = psycopg.connect(settings.DATABASE_URL)
        dbcur = dbcon.cursor()
