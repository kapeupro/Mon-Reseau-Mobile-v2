...
TYPE_IMPORT = "import statistique couverture departement"
TABLE = "stats_couv_departements"
TABLE_FIELDS = [
    "techno",
    "operateur",
    "departement",
...
class CouvertureStatsDepartement(InsertCsv):
    ...
    def readselected(self):
        ...
        self.createtablestatdepartement()
        response, file_copy = self.readcsvfile()

        if file_copy:
            self.copyfile(file_copy)

        self.updatestatcouvdepartement()

        message = response["message"] + " departement importé avec succès"
    ...
    def updatestatcouvdepartement(self):
    ...
    def updatecodeoperateur(self):
        ...
        query = sql.SQL("""
            ...
            set code_operateur = {schema}.arcep_get_code_operateur(st.operateur, st.departement)
        """).format(
    ...
    def createtablestatdepartement(self):
        dbcon = psycopg.connect(settings.DATABASE_URL)
        dbcur = dbcon.cursor()
        query = sql.SQL("""
            ...
            (
                ...
                departement character varying COLLATE pg_catalog."default",
                couv_nc numeric,
                couv_cl numeric,
                couv_bc numeric,
                couv_tbc numeric,
